# AVAS Routing SW — User Manual (v1.3.0)
...
3. **Multi-Link Management**: Dedicated interface to inspect and configure **Single-Link vs Dual-Link** operation on AVAS-223 TX units, track companion chip (`chip_1`) online/offline health in real time, and execute safe synchronized mode switches alongside live video streams.
4. **Touchscreen Ergonomics**: Full touch optimization with 36px–44px targets, native direct touch flick/drag scrolling (`PanningMode="VerticalOnly"`), tactile pressed states (`IsPressed`), and a 14px thumb-resizable splitter handle.
...
```
+-----------------------------------------------------------------------------------------------+
| AVAS ROUTING SW [v1.3.0] | SDVoE Preview Controller              URL: http://localhost:80 [🔄] [⚙] |
+---------------------------------------------------------------------------------------+-------+
|                                                                                       | ◀     |
|                                                                                       | P     |
|                                                                                       | R     |
...
|                                                                                       | W     |
+---------------------------------------------------------------------------------------+-------+
| Ready | Portable WebView2 UserData: .\WebView2_UserData            SDVoE Server: 127.0.0.1:8090|
+-----------------------------------------------------------------------------------------------+
```

### 4.1 Top Navigation Bar
- **App Title & Version Badge**: Displays the active application version (`v1.3.0`).
...
### 4.3 Collapsible Right-Side Sidebar
- **Toggle Strip**: A 28px vertical bar docked on the right side of the screen labeled `◀ PREVIEW`.
- **Minimized by Default**: On startup, the sidebar is completely collapsed to avoid occluding the web canvas and prevent unwanted network multicast traffic.
- **Top Segmented Tab Navigation**:
  - **`📺 Live Previews`**: Real-time video preview cards with live rolling FPS telemetry, IP/port info, and link mode indicator badges (`[SINGLE]` or `[DUAL]`).
  - **`🔗 Multi-Link`**: Dedicated multi-link management cards showing primary MAC, companion chip (`chip_1`) MAC, online/offline status pill (`ONLINE` green / `OFFLINE` red), and Single/Dual link mode toggle controls.
- **Safe Mode Switching & Automatic Reconnect**: Toggling link mode initiates a non-blocking 20-second countdown with progress indicator while the encoder reboots. The preview stream for that encoder is safely paused during reboot and automatically reconnected upon completion, leaving all other preview streams completely unaffected.
- **Collapsing the Sidebar**: Clicking `▶ CLOSE` immediately drops all IGMP multicast memberships and halts background stream processing, freeing 100% of network preview bandwidth.
...
| :--- | :--- | :--- |
| **Theme** | `Light Mode` | User interface visual theme (Light or Dark). |
| **BlueRiver URL** | `http://localhost:80` | The web URL where BlueRiver AV Manager is hosted. |
| **Control Server Host** | `127.0.0.1` | The IP address or hostname of the SDVoE Control Server. |
| **REST API Port** | `8090` | The HTTP REST API port configured in `controlserver.conf` (default 8090). |
| **Telnet TCP Port** | `6970` | Fixed standard SDVoE CLI command port (default `6970`). |
| **Start Multicast IP** | `224.1.3.1` | Starting boundary of the dynamic multicast allocation pool. |
| **End Multicast IP** | `224.1.3.225` | Ending boundary of the dynamic multicast allocation pool. |
| **Multicast Base Port** | `5000` | Base UDP port assigned to preview multicast datagrams. |
